...
\item Recall that the number $e = 2.718281\dots$ is an irrational
  ...
  \item\label{subprob:e_balance_continuous_compounding}
    ...
    number of years.  Calculate your account balance at the end of the
    first year if interest is continuously compounded.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
...
\item The mathematical formulae that govern compound interest are also
  found in the growth of plants.  In~1929, Eric Ashby showed via a
  number of experiments that the growth of duckweeds followed the
  formula for continuous compounding.\footnote{
    See the following paper for further details:
  ...
  \item\label{subprob:duckweeds_growth_factor}
    From \Table{tab:duckweed_frond12_missing}, suppose that the
    initial number of fronds is $a = 103$.  Starting from the second
    day onwards, define the \emph{daily growth factor} as the number
    of fronds of the current day divided by the number of fronds of
    the previous day.  Use this procedure to fill in the missing
    entries in \Table{tab:duckweed_frond12_missing}.  The daily growth
    factor for day~$1$ should be blank.
...
See \Table{tab:duckweed_frond12_complete}.
...
\caption{%%
  A comparison of the experimental frond numbers versus the predicted
...
See \Table{tab:frond12_compare_mean_Ashby} and
\Figure{fig:frond12_theory_data}.

\begin{table}[!htbp]
\centering
\input{table/11/frond-12-comparison.tex}
\caption{%%
  Comparison of the experimental frond numbers versus the frond
  numbers as calculated using
  \Equations{eqn:frond_numbers_Ashby_formula}{eqn:fronds_formula_average_growth_factor}.
  The frond numbers as calculated by
  \Equation{eqn:fronds_formula_average_growth_factor} are given in the
  column with the heading ``Mean growth rate''.  Similarly, the frond
  numbers as calculated by Ashby's
  \Equation{eqn:frond_numbers_Ashby_formula} are listed in the column
  with the heading ``Ashby's formula''.
}
