...
\begin{solution}
\solutionpart{subeg:exponential:Australian_population_2017_growth_rate}
...
\solutionpart{subeg:exponential:Australian_population_2017_initial_population}
...
\solutionpart{subeg:exponential:Australian_population_2017_table_graph}
...
From \Part{subeg:exponential:Australian_population_2017_growth_rate}
you know that the growth rate is $0.016$ per annum and
from \Part{subeg:exponential:Australian_population_2017_initial_population}
...
\Table{tab:exponential:Australian_population_2017}, a graph of which is
illustrated in \Figure{fig:exponential:Australian_population_2017}.
\end{solution}
...
\caption{%%
  ...
  \Table{tab:exponential:Australian_population_2017}.
}
...
\Equation{eqn:exponential:Australian_population_2018_calculation}, in
...
%%
You can see from
\Expression{eqn:exponential:Australian_population_formula} that the
...
\begin{exercise}
Use \Expression{eqn:exponential:Australian_population_formula} to
verify the population numbers in
\Table{tab:exponential:Australian_population_2017}.
\end{exercise}

\ifbool{showSolution}{
\begin{solution}
In \Table{tab:exponential:Australian_population_2017}, the year $2017$
corresponds to $t = 0$ years since $2017$ so according to
\Expression{eqn:exponential:Australian_population_formula} the
population in $2017$ is projected to be
\[
24.7029 \times 1.016^0
=
24.7029
\]
million people.  The year $2018$ corresponds to $t = 1$ year since
$2017$ so the population in $2018$ is projected to be
\[
24.7029 \times 1.016^1
=
25.0981464
\]
million people.  The year $2019$ corresponds to $t = 2$ years since
$2017$ so the population in $2019$ is predicted to be
\[
24.7029 \times 1.016^2
=
25.4997167424
\]
million people.  The year $2020$ corresponds to $t = 3$ years since
$2017$ so the population in $2020$ is projected to be approximately
\[
24.7029 \times 1.016^3
=
25.9077122102784
\]
million people.  Finally, the year $2021$ corresponds to $t = 4$ years
since $2017$ so the population in $2021$ is predicted to be
approximately
\[
24.7029 \times 1.016^4
=
...
\label{sec:exponential:exponential_growth}

Let's derive a general formula to model situations such as those
presented in \Example{eg:exponential:Australian_population_2017}.
Denote by $a$ the initial value of the population and assume that
$a \neq 0$.  Let $r$ be the decimal representation of the percentage
rate of change and define the growth factor by $b = 1 + r$.  For
example, if the population grows by a constant percentage of $2.3\%$,
the decimal representation of this percentage rate of change is
...
at time $t = 0$ you have the initial population of $a$.
...
\Theorem{thm:exponential:exponential_growth}.
\Figure{fig:exponential:general_exponential_growth} shows a general
...
\item\label{subeg:exponential:bacteria_growth_1_2_hours}
  ...
  \Expression{eqn:exponential:Australian_population_2018_calculation}
...
\item\label{subeg:exponential:bacteria_growth_formula}
  ...
  results from \Part{subeg:exponential:bacteria_growth_1_2_hours}.  Produce a
...
\begin{solution}
\solutionpart{subeg:exponential:bacteria_growth_rate}
...
\solutionpart{subeg:exponential:bacteria_initial_population}
The initial bacteria population is $a = 10$ cells.

\solutionpart{subeg:exponential:bacteria_growth_1_2_hours}
...
\solutionpart{subeg:exponential:bacteria_growth_formula}
From \Parts{subeg:exponential:bacteria_growth_rate}{subeg:exponential:bacteria_initial_population},
...
\Theorem{thm:exponential:exponential_growth} to write
%%
\begin{equation}
...
from \Part{subeg:exponential:bacteria_growth_1_2_hours}.  At time
$t = 1$ hour, you have
\[
Q(1)
=
...
in \Part{subeg:exponential:bacteria_growth_1_2_hours}.
\Figure{fig:exponential:bacteria_population_24_hours} shows a plot of
...
\begin{exercise}
This is a continuation of \Example{eg:exponential:bacterial_growth}.
%%
\begin{packedenum}
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:bacteria_100_cells}
Using \Equation{eqn:exponential:bacterial_growth}, after $t = 5$ hours
the bacteria population would be approximately
\[
Q(5)
=
10 \times 1.56^5
...
\solutionpart{subex:exponential:bacteria_doubling_time}
...
\Equation{eqn:exponential:bacterial_growth} shows that after
approximately $1.2857$ hours the number of bacteria cells has
increased to
\[
Q(1.2857)
=
...
you substitute $t = 2$ hours into
\Equation{eqn:exponential:bacterial_growth}, you obtain
\[
Q(2)
=
10 \times 1.56^2
...
\item\label{subex:exponential:Sweden_population_formula_graph}
  ...
  from \Part{subex:exponential:Sweden_population_2019_2020}.  Produce
...
\item\label{subex:exponential:Sweden_population_doubling_time}
  ...
  formula
  from \Part{subex:exponential:Sweden_population_formula_graph} and
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:Sweden_population_growth_rate_factor}
...
\solutionpart{subex:exponential:Sweden_population_2019_2020}
...
\label{fig:exponential:population_Sweden}
\end{figure}

\solutionpart{subex:exponential:Sweden_population_formula_graph}
You assume that the population of $a = 10.135303$ million people in
2018 is the initial population.
From \Part{subex:exponential:Sweden_population_growth_rate_factor} you
...
in \Part{subex:exponential:Sweden_population_2019_2020}.
\Figure{fig:exponential:population_Sweden} shows a graph of the
projected population of Sweden from 2018 up to and including the year
2030.

\solutionpart{subex:exponential:Sweden_population_doubling_time}
...
\Equation{eqn:exponential:Sweden_population_formula} shows that after
...
\Equation{eqn:exponential:Sweden_population_formula} you get
\[
Q(59)
=
...
\Equation{eqn:exponential:Sweden_population_formula} again shows that
\[
Q(58)
=
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:E_coli_formula_graph}
...
Let $t$ be the number of doublings, i.e.~the number of times that
doubling occurred.  Use \Theorem{thm:exponential:exponential_growth}
to write the population $Q(t)$ after $t$ doublings as
%%
\begin{equation}
\label{eqn:exponential:ecoli_growth}
Q(t)
=
5 \times 2^t.
\end{equation}
...
would have undergone $12$ doublings.
\Figure{fig:exponential:ecoli_growth} shows a graph of the growth of
the \emph{E.~coli} population.
...
\caption{%%
  ...
  of doublings.  Multiply the number $t$ of doublings by $24$ to
  obtain $24t$ minutes.
}
\label{fig:exponential:ecoli_growth}
\end{figure}

\solutionpart{subex:exponential:E_coli_1000_cells}
Substitute $t = 7$ into \Equation{eqn:exponential:ecoli_growth} and
you get
\[
Q(7)
=
...
\end{equation}
%%
You can derive \Equation{eqn:exponential:exponential_decay} by using
the same reasoning that led to
\Equation{eqn:exponential:exponential_growth}.  Note that
\Equation{eqn:exponential:exponential_decay} is almost identical to
the equation for exponential growth in
\Theorem{thm:exponential:exponential_growth}.  The above discussion is
summarised in \Theorem{thm:exponential:exponential_decay}.
\Figure{fig:exponential:exponential_decay} shows a graph of what an
...
\Example{eg:exponential:water_level} clarifies this point and the rule
of $69$ is summarised in \Theorem{thm:exponential:rule_of_69}.
...
\begin{solution}
\solutionpart{subex:exponential:water_level_formula}
...
\Theorem{thm:exponential:exponential_decay} to write
%%
\begin{equation}
...
\end{equation}
%%
See \Figure{fig:exponential:bucket_water_level} for a graph of the
amount of water in the bucket up to $70$ minutes.

\solutionpart{subex:exponential:water_level_half_life}
...
\Equation{eqn:exponential:bucket_water_level} yields
\[
W(12)
=
3 \times 0.944^{12}
...
$t = 13$ into \Equation{eqn:exponential:bucket_water_level} and you get
\[
W(13)
=
3 \times 0.944^{13}
...
\begin{solution}
\solutionpart{subeg:exponential:carbon14_decay_rate}
...
\solutionpart{subeg:exponential:carbon14_decay_factor}
...
\solutionpart{subeg:exponential:carbon14_after_1000_and_2000_years}
...
\solutionpart{subeg:exponential:carbon14_formula_graph}
...
of time, use \Theorem{thm:exponential:exponential_decay} to write
...
\Figure{fig:exponential:carbon14_decay} shows a graph of the amount of
...
\caption{%%
  The number of isotopes of carbon-$14$ remaining after
  $t \times 1000$ years.  You initially have $300$ isotopes of
  carbon-$14$.  It is assumed that the constant percentage decay
  rate of carbon-$14$ is $11.4\%$ per $1000$ years.
}
\label{fig:exponential:carbon14_decay}
\end{figure}

\begin{exercise}
Use the formula from the solution of
\Subexample{eg:exponential:radiocarbon_dating}{subeg:exponential:carbon14_formula_graph}
to verify the solution in
\Subexample{eg:exponential:radiocarbon_dating}{subeg:exponential:carbon14_after_1000_and_2000_years}.
\end{exercise}

\ifbool{showSolution}{
\begin{solution}
Suppose the initial amount of carbon-$14$ is $300$ isotopes and that
carbon-$14$ decreases at a constant percentage rate of $11.4\%$ per
$1000$ years.  From
\Subexample{eg:exponential:radiocarbon_dating}{subeg:exponential:carbon14_formula_graph},
the number $A(t)$ of isotopes of carbon-$14$ remaining after
...
265.8
\]
isotopes.  After $2000$ years, you have $t = 2$.  Substitute $t = 2$
...
235.4988
\]
isotopes.  These numbers are the same as those obtained in the
solution of
\Subexample{eg:exponential:radiocarbon_dating}{subeg:exponential:carbon14_after_1000_and_2000_years}.
\end{solution}
}{}
...
\item\label{subex:exponential:fermium250_decay_formula}
  ...
  from \Part{subex:exponential:fermium250_30minutes_60minutes}.
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:fermium250_decay_rate}
Since the constant percentage decay rate is $50\%$, the constant decay
rate is $r = 50 / 100 = 0.5$.  This means that after every $30$
minutes the amount of fermium-$250$ decreases by a rate of $0.5$.

\solutionpart{subex:exponential:fermium250_decay_factor}
The decay factor is defined as one minus the decay rate so
$b = 1 - 0.5 = 0.5$.  Multiply this number by $100\%$ to get
$0.5 \times 100\% = 50\%$.  After every $30$ minutes only $50\%$ of
the original amount of fermium-$250$ remains.

\solutionpart{subex:exponential:fermium250_30minutes_60minutes}
...
$100$ isotopes of fermium-$250$ at time $t = 0$.  At time $t = 1$ or
after $30$ minutes, the quantity of fermium-$250$ would have decreased
by $100 \times 0.5$ isotopes.  Subtract this number from the amount at
time $t = 0$ to get
%%
\begin{align*}
100 - 100 \times 0.5
...
\end{align*}
%%
isotopes of fermium-$250$ remaining.  At time $t = 2$ or after an
hour, the quantity of fermium-$250$ would have decreased by
$50 \times 0.5$ isotopes.  Subtract this number from the amount at
time $t = 1$ to get
%%
\begin{align*}
50 - 50 \times 0.5
...
\end{align*}
%%
isotopes of fermium-$250$ remaining.
...
\solutionpart{subex:exponential:fermium250_decay_formula}
...
$t$ units of time, use \Theorem{thm:exponential:exponential_decay} to
write $A(t) = 100 \times 0.5^t$.  Let's use the latter formula to
verify the results
from \Part{subex:exponential:fermium250_30minutes_60minutes}.  The
...
obtained in \Part{subex:exponential:fermium250_30minutes_60minutes}.
\Figure{fig:exponential:fermium250_decay} shows a graph of the
decrease in the amount of fermium-$250$ after every $30$ minutes.

\solutionpart{subex:exponential:fermium250_less_than_1microgram}
...
\item\label{subex:exponential:soup_temperature_formula_graph}
  ...
  from \Part{subex:exponential:soup_temperature_1minute_2minutes}.
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:soup_temperature_1minute_2minutes}
...
\solutionpart{subex:exponential:soup_temperature_formula_graph}
From \Part{subex:exponential:soup_temperature_1minute_2minutes} you
know that the initial temperature of the pot of soup is $a = 100$
degrees Celsius and the constant rate of cooling is $r = 0.018$.  Then
the cooling factor~(i.e.~the decay factor) is $b = 1 - 0.018 = 0.982$.
If $T(t)$ represents the temperature~(in degrees Celsius) of the pot
of soup after $t$ minutes, use
\Theorem{thm:exponential:exponential_decay} to write
%%
\begin{equation}
\label{eqn:exponential:soup_temperature}
T(t)
=
100 \times 0.982^t.
\end{equation}
...
in \Part{subex:exponential:soup_temperature_1minute_2minutes}.
\Figure{fig:exponential:soup_temperature} shows a graph of the
temperature of the pot of soup up to after $180$ minutes or three
hours.
...
\solutionpart{subex:exponential:soup_temperature_half_life}
...
$t = 38$ into \Equation{eqn:exponential:soup_temperature} results in
\[
T(38)
=
100 \times 0.982^{38}
...
\solutionpart{subex:exponential:soup_temperature_15_degrees}
...
\item\label{subex:exponential:MnM_probability}
  ...
  with the graph from \Part{subex:exponential:MnM_data}.  What do you
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:MnM_data}
\Table{tab:exponential:MnM_data} shows the data for one run of the
M\&M's experiment.  \Figure{fig:exponential:MnM_graph} shows a graph
of the number of remaining M\&M's after $t$ minutes.  The figure shows
that the ``decay'' of the M\&M's seems to follow a model of
exponential decay.

\begin{table}[!htbp]
\centering
\input{table/11/ms.tex}
\caption{%%
  The ``decay'' of $100$ M\&M's.  Going from left to right, the first
  column lists how many times M\&M's have been poured onto a flat
  surface.  The middle column shows after the $t$-th throw the number
  of M\&M's that were moved to the ``decayed'' pile, not the
  cumulative sum that were moved to the ``decayed'' pile.  Finally,
  the third column lists the number of M\&M's remaining, i.e.~those
  that are yet to be moved to the ``decayed'' pile.
}
\label{tab:exponential:MnM_data}
\end{table}

\begin{figure}[!htbp]
\centering
\includegraphics[scale=1.1]{image/11/ms.pdf}
\caption{%%
  The number of remaining M\&M's as a function of time.  Here time is
  measured in minutes.  Originally, there were $100$ M\&M's.  The
  black dots show the experimental data from
  \Table{tab:exponential:MnM_data}.  The blue curve is a graph of
  \Equation{eqn:exponential:MnM_probability_model}.
}
\label{fig:exponential:MnM_graph}
\end{figure}

\solutionpart{subex:exponential:MnM_decay_factor}
If the ``decay'' of the M\&M's were to follow an exponential decay
model, then the decay factor of the model should be a constant that is
approximately $0.5$.  The reason is that after each throw you expect
to move $50\%$ of the M\&M's to the ``decayed'' pile.  The other
$50\%$ are considered the remaining M\&M's.

\solutionpart{subex:exponential:MnM_probability}
...
\end{equation}
%%
\Figure{fig:exponential:MnM_graph} shows a graph of
\Equation{eqn:exponential:MnM_probability_model}.  The figure shows
...
prediction of \Equation{eqn:exponential:MnM_probability_model} differs
from the experimental data by approximately $1.7989$.  Such a
relatively low value of the RMSE indicates that
\Equation{eqn:exponential:MnM_probability_model} is fairly good at
...
possible.  \Example{eg:exponential:savings_6months} below should help
...
\item\label{subeg:exponential:savings_6months_balance_formula}
  ...
  from \Part{subeg:exponential:savings_6months_balance_first_year}.
...
\begin{solution}
\solutionpart{subeg:exponential:savings_6months_initial_balance}
Your account has an initial balance of $\$100$.

\solutionpart{subeg:exponential:savings_6months_growth_rate}
...
\solutionpart{subeg:exponential:savings_6months_balance_first_year}
...
\solutionpart{subeg:exponential:savings_6months_balance_formula}
...
Substitute $t = 1$ into
\Equation{eqn:exponential:compound_interest_a100_b1.015} to see that
...
\Equation{eqn:exponential:compound_interest_a100_b1.015} shows that at
...
from \Part{subeg:exponential:savings_6months_balance_first_year}.
\Figure{fig:exponential:compound_interest_a100_b1.015} shows a graph
...
\solutionpart{subeg:exponential:savings_6months_doubling_time}
...
\Equation{eqn:exponential:compound_interest_a100_b1.015} and you have
\[
B(48)
=
100 \times 1.015^{48}
...
\Equation{eqn:exponential:compound_interest_a100_b1.015} yields
\[
B(47)
=
100 \times 1.015^{47}
...
In \Example{eg:exponential:savings_6months}, how much interest~(in
...
\begin{solution}
By
\Subexample{eg:exponential:savings_6months}{subeg:exponential:savings_6months_balance_first_year},
...
Suppose that in \Example{eg:exponential:savings_6months} interest is
...
\item\label{subex:exponential:savings_1year_formula}
  ...
  \Subexample{eg:exponential:savings_6months}{subeg:exponential:savings_6months_balance_formula}
  as a comparison.
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:savings_1year_formula}
From \Example{eg:exponential:savings_6months}, the initial balance is
$a = 100$ dollars and the interest rate is $3\%$ per annum.  Since
interest is compounded once per year, the constant growth rate is
$r = 3 / 100 = 0.03$ per annum and the growth factor is
$b = 1 + 0.03 = 1.03$.  If $B(t)$ represents the account balance after
$t$ compounding periods, use
\Theorem{thm:exponential:exponential_growth} to write
%%
\begin{equation}
\label{eqn:exponential:savings_1year_compounding}
B(t)
=
100 \times 1.03^t.
\end{equation}
...
year.  \Figure{fig:exponential:savings_1year_6months} shows a
comparison of the account balance when interest is calculated once per
year as opposed to twice per year.  The graph shows that when interest
is calculated twice per year, the balance at the end of each
compounding period is slightly higher than when interest is compounded
once annually.
...
\label{fig:exponential:savings_1year_6months}
\end{figure}

\solutionpart{subex:exponential:savings_1year_interest}
Using \Equation{eqn:exponential:savings_1year_compounding}, the
...
\solutionpart{subex:exponential:savings_1year_doubling_time}
...
\Equation{eqn:exponential:savings_1year_compounding} to get
\[
B(24)
=
100 \times 1.03^{24}
...
\Equation{eqn:exponential:savings_1year_compounding} shows that
\[
B(23)
=
100 \times 1.03^{23}
...
\item\label{subeg:exponential:savings_quarterly_formula}
  ...
  \Subexample{eg:exponential:savings_6months}{subeg:exponential:savings_6months_balance_formula}
  and
  \Subexercise{ex:exponential:savings_once_annually}{subex:exponential:savings_1year_formula}
  as comparison.
...
\begin{solution}
\solutionpart{subeg:exponential:savings_quarterly_formula}
...
\Figure{fig:exponential:savings_quarterly} for up to the first three
years since opening the account.  As a comparison, the figure also
shows the graphs of
\Equation{eqn:exponential:compound_interest_a100_b1.015} and of the
equation from
\Subexercise{ex:exponential:savings_once_annually}{subex:exponential:savings_1year_formula}.
...
\solutionpart{subeg:exponential:savings_quarterly_doubling_time}
...
\Equation{eqn:exponential:savings_quarterly_compounding} you would get
\[
B(93)
=
100 \times 1.0075^{93}
...
\Equation{eqn:exponential:savings_quarterly_compounding} yields
\[
B(92)
=
100 \times 1.0075^{92}
...
\begin{exercise}
\textbf{How much interest?}
In \Example{eg:exponential:savings_quarterly_compounding}, how much
...
into \Equation{eqn:exponential:savings_quarterly_compounding} to see
that your account balance at the end of the first year is
approximately
\[
B(4)
=
...
\item\label{subex:exponential:savings_monthly_formula}
  ...
  \Subexercise{ex:exponential:savings_once_annually}{subex:exponential:savings_1year_formula},
  and
  \Subexample{eg:exponential:savings_quarterly_compounding}{subeg:exponential:savings_quarterly_formula}.
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:savings_monthly_formula}
The initial balance is $a = 100$ dollars and the interest rate is
...
$t$ months, use \Theorem{thm:exponential:exponential_growth} to write
$B(t)$ as
%%
\begin{equation}
\label{eqn:exponential:savings_monthly_balance}
B(t)
=
100 \times 1.0025^t.
\end{equation}
%%
\Figure{fig:exponential:savings_monthly_comparison} shows a comparison
of the account balance from the $30$-th month up to the $36$-th month
...
\label{fig:exponential:savings_monthly_comparison}
\end{figure}

\solutionpart{subex:exponential:savings_monthly_interest}
Each compounding period corresponds to one month, so within a year you
have $12$ compounding periods.  Substitute $t = 12$ into
\Equation{eqn:exponential:savings_monthly_balance} to see that at the
end of the first year your account balance would be approximately
\[
B(12)
=
100 \times 1.0025^{12}
...
\solutionpart{subex:exponential:savings_monthly_doubling_time}
...
$t = 278$ into \Equation{eqn:exponential:savings_monthly_balance}
shows that
\[
B(278)
=
...
$t = 277$ into \Equation{eqn:exponential:savings_monthly_balance} yields
\[
B(277)
=
100 \times 1.0025^{277}
...
\item\label{subex:exponential:Fry_continuous_compounding}
  ...
  of compounding years?  Refer to \Problem{prob:exponential:number_e}
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:exponential:Fry_initial_balance}
Fry had an initial account balance of $\cent{93}$ or $\$0.93$.  The
constant interest rate is $2.25\%$ per annum, so the annual growth
rate is $r = 2.25 / 100 = 0.0225$.  The account balance had been left
to grow for $1000$ years.

\solutionpart{subex:exponential:Fry_formula}
The bank teller used the following formula to approximate Fry's
account balance:
\[
B(t)
=
...
formula, Fry's account balance after $t = 1000$ years is approximately
\[
B(1000)
=
...
\solutionpart{subex:exponential:Fry_doubling_time}
The rule of $72$ tells you that the doubling time of Fry's account
balance is approximately $72 / 2.25 = 32$ years.  Note that twice the
initial balance is $2 \times 0.93 = 1.86$ dollars.  After $t = 32$
years, Fry's account balance would be
\[
B(32)
=
0.93 \times 1.0225^{32}
...
1.90
\]
dollars, rounded to the nearest cent.  However, after $t = 31$ years
...
1.85
\]
dollars, also rounded to the nearest cent.  You may conclude that the
doubling time on Fry's account balance is at most $32$ years.

\solutionpart{subex:exponential:Fry_continuous_compounding}
If interest had been continuously compounded, Fry's account balance at
the end of $1000$ years would be
\[
...
\item\emph{Chessboard.}
  ...
  \Figure{fig:exponential:chessboard}, going from left to right, top
  ...
  \item\label{subprob:exponential:sand_neurons_human_brain}
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:sand_formula}
Let $t \geq 0$ be the number of times that you perform the doubling
process.  Going from left to right, top to bottom on the chessboard in
\Figure{fig:exponential:chessboard} you may identify each square with
an integer from $1$ up to $64$, inclusive.  You start off with one
grain of sand on the first square so the number of times you have
performed the doubling process is zero.  You perform the doubling
process once to obtain the number of grains of sand for the second
square.  To obtain the number of grains of sand for the third square,
you must perform the doubling process two times.  In general, to
obtain the number of grains of sand for the $n$-th square you must
perform the doubling process a total of $t = n - 1$ times.

Let $G(t)$ represent the number of grains of sand on square number
$t + 1$.  For example, you have $G(0) = 1 = 2^0$ grains of sand on
square number $0 + 1 = 1$.  You have $G(1) = 2 = 2^1$ grains of sand
on square number $1 + 1 = 2$.  On square number $2 + 1 = 3$, you have
$G(2) = 4 = 2^2$ grains of sand.  You have $G(3) = 8 = 2^3$ grains of
sand on square number $3 + 1 = 4$.  Furthermore, you have
...
factor is $b = 1 + 1 = 2$.  Using
\Theorem{thm:exponential:exponential_growth}, you can write
\[
G(t)
=
ab^t
...
\solutionpart{subprob:exponential:sand_neurons_human_brain}
For $t = 36$ you have
\[
G(36)
=
...
\item\label{prob:exponential:number_e}
  ...
  \item\label{subprob:exponential:e_balance_up_to_12_compounding}
    You can use
    \Equation{eqn:exponential:e_savings_first_year_n_compounding} to
  ...
  \item\label{subprob:exponential:e_balance_limit_of_growth_factor_balance}
    Based on
    \Equation{eqn:exponential:e_savings_first_year_n_compounding} and
    your graphs
    from \Part{subprob:exponential:e_balance_up_to_1200_compounding},
  ...
  \item\label{subprob:exponential:e_balance_continuous_compounding}
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:e_balance_formula}
The initial balance is $a = 1$ dollar.  Since there are $n$
compounding periods per year and the interest rate is $100\%$ per
annum, at each compounding period the rate you use for interest
calculation is $100 / n$ percent.  Divide this number by $100$ to get
the constant growth rate of
\[
r
=
...
period, use \Theorem{thm:exponential:exponential_growth} to write
%%
\begin{align*}
B(t)
&=
...
\solutionpart{subprob:exponential:e_balance_up_to_12_compounding}
See \Table{tab:exponential:e_balance_up_to_12_compounding} and
\Figure{fig:exponential:e_balance_up_to_12_compounding}.
\Figure{subfig:exponential:e_compoundings_vs_growth_factor} shows that
as the number of compoundings per annum increases, the growth factor
decreases.  Furthermore,
\Figure{subfig:exponential:e_compoundings_vs_balance} shows that as
the number of compoundings per annum increases the account balance at
the end of the year increases.

\begin{table}[!htbp]
\centering
\input{table/11/e-12.tex}
\caption{%%
...
\subfigure[]{
  ...
  \label{subfig:exponential:e_compoundings_vs_growth_factor}
}
%%
%%
\subfigure[]{
  \includegraphics[scale=1.1]{image/11/e-12-balance.pdf}
  \label{subfig:exponential:e_compoundings_vs_balance}
}
\caption{%%
  ...
  \Table{tab:exponential:e_balance_up_to_12_compounding}.
}
\label{fig:exponential:e_balance_up_to_12_compounding}
\end{figure}

\solutionpart{subprob:exponential:e_balance_up_to_1200_compounding}
See \Figure{fig:exponential:e_savings_up_to_1200_compounding}.
\Figure{subfig:e_savings_1200_growth_factor} shows that as the number
...
\Figure{subfig:e_savings_1200_balance} shows that as the number of
...
\solutionpart{subprob:exponential:e_balance_limit_of_growth_factor_balance}
...
\Figure{fig:exponential:e_saving_1200_growth_rate}.  That is, as $n$
gets larger and larger the growth factor $g(n)$ gets closer and closer
to the value of $1$; see
\Figure{subfig:e_savings_1200_growth_factor}.  Therefore the account
balance $B(n)$ at the end of the first year gets closer and closer to
a fixed number.  From \Figure{subfig:e_savings_1200_balance}, the
...
\solutionpart{subprob:exponential:e_balance_continuous_compounding}
...
\begin{table}[!htbp]
\centering
\input{table/11/frond-12-missing.tex}
\caption{%%
...
\item\label{prob:exponential:duckweeds}
  ...
  \Table{tab:exponential:duckweed_frond12_missing}.
  %%
  \begin{packedenum}
  \item\label{subprob:exponential:duckweeds_growth_factor}
    Graph the data in \Table{tab:exponential:duckweed_frond12_missing}
    as a scatter plot.  From
    \Table{tab:exponential:duckweed_frond12_missing}, suppose that the
    ...
    in \Table{tab:exponential:duckweed_frond12_missing}.  The daily
    growth factor for day~$1$ should be blank.  If the data in
    \Table{tab:exponential:duckweed_frond12_missing} perfectly follow
  ...
  \item\label{subprob:exponential:duckweeds_growth_factor_estimate}
    ...
    from \Part{subprob:exponential:duckweeds_growth_factor}, you can
  ...
  \item\label{subprob:exponential:duckweeds_growth_factor_exponential}
    ...
    \Table{tab:exponential:duckweed_frond12_missing}.  The third
    column should have the header ``$F(t)$'' and lists the frond
    numbers as calculated using your formula
    from \Part{subprob:exponential:duckweeds_growth_factor_estimate}.
    Finally, the fourth column should have the header ``Ashby'' and
    lists the frond numbers as calculated using Ashby's
    \Equation{eqn:exponential:frond_numbers_Ashby_formula}.  Add to
    the graph
    in \Part{subprob:exponential:duckweeds_growth_factor_estimate} a
    graph of \Equation{eqn:exponential:frond_numbers_Ashby_formula}.

  \item\label{subprob:exponential:duckweeds_error_analysis}
    For each observed frond number in
    \Table{tab:exponential:duckweed_frond12_missing}, calculate the
    errors that result from using your formula
    from \Part{subprob:exponential:duckweeds_growth_factor_estimate}
    to predict the frond number.  Repeat the error calculation for
    \Equation{eqn:exponential:frond_numbers_Ashby_formula}.  Produce a
    graph of the day number versus the errors.  What do you notice?
    By using the root mean square error, explain which of
    \Equation{eqn:exponential:frond_numbers_Ashby_formula} and your
    formula
    from \Part{subprob:exponential:duckweeds_growth_factor_estimate}
    is better at modelling the frond numbers listed in
    \Table{tab:exponential:duckweed_frond12_missing}.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:duckweeds_growth_factor}
See \Figure{fig:exponential:frond12_theory_data} and
\Table{tab:exponential:duckweed_frond12_complete}.  If the data in
\Table{tab:exponential:duckweed_frond12_missing} were to perfectly
follow the model of exponential growth, then the daily growth factors
for the data should all be the same.  In fact, the daily growth
factors would be the same as the growth factor of an exponential
growth model.  However, as shown in
\Table{tab:exponential:duckweed_frond12_complete} the daily growth
factors for the data are not all the same.  The daily growth factors
are similar in terms of the first decimal digits, but they are not
approximately the same in the second decimal digits.  You may conclude
from the daily growth factors that the data in
\Table{tab:exponential:duckweed_frond12_missing} do not perfectly
follow an exponential growth model.  However, the daily growth factors
are nearly the same in the first decimal digits, and the scatter plot
of the data seems to show an exponential-like growth, that you may
attempt to model the data with a formula for exponential growth.

\begin{table}[!htbp]
\centering
\input{table/11/frond-12-complete.tex}
\caption{%%
  The experimental number of fronds in a colony of duckweeds, counted
  each day for nine consecutive days.  Before the first day, the
  colony had been exposed to $12$ hours of light per day for three
  consecutive days.  This is the same as
  \Table{tab:exponential:duckweed_frond12_missing} except that missing
  entries have been filled in.  All numbers in the column for daily
  growth factor have been rounded to six decimal places.
}
\label{tab:exponential:duckweed_frond12_complete}
\end{table}

\solutionpart{subprob:exponential:duckweeds_growth_factor_estimate}
...
substitute $t = 1$ into
\Equation{eqn:exponential:fronds_formula_average_growth_factor}, then
...
\end{align*}
%%
\Figure{fig:exponential:frond12_theory_data} shows a graph of
\Equation{eqn:exponential:fronds_formula_average_growth_factor}
...
\caption{%%
  ...
  \Equations{eqn:exponential:frond_numbers_Ashby_formula}{eqn:exponential:fronds_formula_average_growth_factor}.
  The black dots represent the experimental data from
  \Table{tab:exponential:duckweed_frond12_missing}.  The blue curve is
  a graph of
  \Equation{eqn:exponential:fronds_formula_average_growth_factor},
  which is based on the mean growth rate.  The red curve is a graph of
  Ashby's \Equation{eqn:exponential:frond_numbers_Ashby_formula}.
}
\label{fig:exponential:frond12_theory_data}
\end{figure}

\solutionpart{subprob:exponential:duckweeds_growth_factor_exponential}
\Table{tab:exponential:frond12_compare_mean_Ashby} and
\Figure{fig:exponential:frond12_theory_data} compare the observed
frond numbers with the predicted frond numbers.

\begin{table}[!htbp]
\centering
\input{table/11/frond-12-comparison.tex}
\caption{%%
  Comparison of the observed frond numbers versus the predicted frond
  numbers.  The predicted frond numbers as calculated by
  \Equations{eqn:exponential:fronds_formula_average_growth_factor}{eqn:exponential:frond_numbers_Ashby_formula}
...
\caption{%%
  ...
  \Equation{eqn:exponential:fronds_formula_average_growth_factor} and
  ...
  \Table{tab:exponential:duckweed_frond12_missing}, where $d_i$
  denotes the number of fronds observed during day $i$.  The
  prediction of
  \Equation{eqn:exponential:fronds_formula_average_growth_factor} for
  ...
  \Equation{eqn:exponential:fronds_formula_average_growth_factor} are
...
\solutionpart{subprob:exponential:duckweeds_error_analysis}
\Figure{fig:exponential:frond_error_analysis} compares the errors that
result from using
\Equation{eqn:exponential:fronds_formula_average_growth_factor} and
...
\Equation{eqn:exponential:fronds_formula_average_growth_factor} is
approximately $4.8256$, which is smaller than the RMSE of $8.2572$ of
Ashby's \Equation{eqn:exponential:frond_numbers_Ashby_formula}, both
...
\item\emph{Zero and one.}
  ...
  \item\label{subprob:exponential:binary_to_decimal}
    ...
    Use \Equation{eqn:exponential:binary_to_decimal_formula} to
  ...
  \item\label{subprob:exponential:binary_how_many_integers}
    ...
    \Subproblem{prob:exponential:duckweeds}{subprob:exponential:duckweeds_growth_factor}.

  \item\label{subprob:exponential:binary_how_many_integers_formula}
    Use your results
    from \Part{subprob:exponential:binary_how_many_integers} to derive
    a formula for the number of non-negative integers that can be
    represented with $n$ bits.  Verify your formula with the data in
    the table
    from \Part{subprob:exponential:binary_how_many_integers}.
    Determine the largest non-negative integer that can be represented
    with $n$ bits.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:binary_to_decimal}
Using \Equation{eqn:exponential:binary_to_decimal_formula}, the binary
number $10100101$ can be converted to a non-negative integer as
%%
\begin{align*}
&b_7 2^7 + b_6 2^6 + b_5 2^5 + b_4 2^4
  + b_3 2^3 + b_2 2^2 + b_1 2^1 + b_0 2^0 \\[4pt]
...
\solutionpart{subprob:exponential:binary_4_bits}
See \Table{tab:exponential:binary_4_bits}.

\begin{table}[!htbp]
\centering
\input{table/11/binary-4bits.tex}
\caption{%%
  All possible binary strings with four bits.  Each binary string
  corresponds to a non-negative integer.
}
\label{tab:exponential:binary_4_bits}
\end{table}

\solutionpart{subprob:exponential:binary_how_many_integers}
See \Table{tab:exponential:binary_how_many_integers}.

\begin{table}[!htbp]
\centering
\input{table/11/binary.tex}
\caption{%%
...
\solutionpart{subprob:exponential:binary_how_many_integers_formula}
\Table{tab:exponential:binary_how_many_integers} shows that adding an
...
\item\label{prob:exponential:radioactive_dice}
  ...
  dice, generate experimental data using the dice, and analyse the
  ...
  \item\label{subprob:exponential:radioactive_dice_simulation_roll_dice}
    ...
    %%
    \begin{center}
    \code{roll\_dice(n\comma decay\_state)}
    \end{center}
    %%
    The first argument \code{n} specifies how many dice to roll.  The
    value of \code{n} must be a positive integer.  The second argument
    \code{decay\_state} specifies the number of pips that indicates
    the ``decay'' state of a die.  The purpose of the function
    \code{roll\_dice()} is to roll the given number of dice one at a
    time and keep track of how many dice have ``decayed''.  The
    function should return the number of dice that have ``decayed''
    and the number of dice that are yet to ``decay''.
  ...
  \item\label{subprob:exponential:radioactive_dice_simulation_main}
    ...
    \end{center}
    %%
    The purpose of \code{main()} is to implement the simulation as
    described above.  The function \code{main()} should invoke the
    function \code{roll\_dice()} as many times as necessary until all
    dice have ``decayed''.  Each call to \code{roll\_dice()}
    corresponds to an increment of one minute.  After each invocation
    of \code{roll\_dice()}, the function \code{main()} should note the
    time $t$, the number $d$ of dice that have ``decayed'', and the
    number $r$ of remaining dice that are yet to ``decay''.  At the
    end of the simulation, \code{main()} should write the experimental
    ...
    %%
    \begin{center}
    \code{Time (minutes),How many decayed,How many remaining}
    \end{center}
  ...
  \item\label{subprob:exponential:radioactive_dice_cumulative_decayed}
    ...
    from \Part{subprob:exponential:radioactive_dice_graph_remaining}.
  ...
  \item\label{subprob:exponential:radioactive_dice_decay_constant}
    ...
    decimal places.\footnote{
      ...
      \Equation{eqn:exponential:half_life_related_to_decay_rate}.
    }
    Use your graph
    from \Part{subprob:exponential:radioactive_dice_graph_remaining}
    to estimate the half-life of the ``radioactive'' dice.  Use your
    estimate and
    \Equation{eqn:exponential:half_life_related_to_decay_rate} to
    ...
    from \Part{subprob:exponential:radioactive_dice_cumulative_decayed}.
  ...
  \item\label{subprob:exponential:radioactive_dice_error_analysis}
    For each of the functions $R(t)$, $M(t)$, and $P(t)$ graph the
    errors of the function versus time $t$.  Put all three error
    graphs on one set of axes.  Explain what you notice about the
    graph.  Use the root mean square error to help you explain which
    one of the above three functions is better than the other two at
    modelling your experimental data.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:radioactive_dice_graph_remaining}
\Table{tab:exponential:radioactive_dice} shows the data from one run
of the above experiment with $1000$ ``radioactive'' dice~(the dice
were not actually radioactive).  See
\Figure{fig:exponential:radioactive_dice} for a scatter plot of the
experimental data in \Table{tab:exponential:radioactive_dice}.  The
table shows that after the elapse of $42$ minutes you have zero dice
remaining.  In other words, in this particular run of the experiment
$42$ minutes were required in order for all the $1000$ ``radioactive''
dice to ``decay''.

\begin{table}[!htbp]
\centering
\input{table/11/radioactive-dice.tex}
\caption{%%
...
\label{tab:exponential:radioactive_dice}
\end{table}
...
\caption{%%
  The decay of $1000$ ``radioactive'' dice.  The black dots
  represent the experimental number of remaining ``undecayed'' dice
  after the elapse of the given number of minutes.  The red curve is a
  graph of
  \Equation{eqn:exponential:radioactive_dice_decay_constant_formula},
  which predicts how many dice are yet to ``decay'' after $t$
  minutes.  Similarly, the blue dots represent the experimental
  cumulative number of dice that have ``decayed'' after the elapse of
  a given number of minutes.  This number is predicted by
  \Equation{eqn:exponential:radioactive_dice_total_decayed}, which is
  represented by the green curve.  The experimental data are from
  \Table{tab:exponential:radioactive_dice}.
}
\label{fig:exponential:radioactive_dice}
\end{figure}

\solutionpart{subprob:exponential:radioactive_dice_cumulative_decayed}
See \Table{tab:exponential:radioactive_dice_cumulative_decayed} and
\Figure{fig:exponential:radioactive_dice}.

\begin{table}[!htbp]
\centering
\input{table/11/radioactive-dice-cumulative.tex}
\caption{%%
  This is the same as \Table{tab:exponential:radioactive_dice}, but
...
\solutionpart{subprob:exponential:radioactive_dice_decay_constant}
...
$1000 / 2 = 500$ dice.  \Figure{fig:exponential:radioactive_dice}
...
\Equation{eqn:exponential:half_life_related_to_decay_rate} and you get
...
1000 - R(t) \\[4pt]
&=
1000 - 1000 \times 0.82671325^t \\[4pt]
&=
1000(1 - 0.82671325^t).
\end{aligned}
\end{equation}
...
\solutionpart{subprob:exponential:radioactive_dice_mean_decay_rate}
...
$b \approx 0.830929$.  If $M(t)$ represents the number of dice
remaining after $t$ minutes, you can write
%%
\begin{equation}
...
\solutionpart{subprob:exponential:radioactive_dice_probability}
...
\caption{%%
  ...
  $R(t)$~(\Equation{eqn:exponential:radioactive_dice_decay_constant_formula}),
  $M(t)$~(\Equation{eqn:exponential:radioactive_dice_mean_decay_rate}),
  and
  $P(t)$~(\Equation{eqn:exponential:radioactive_dice_probability}).
  Let $\tuple{\tau}{\rho}$ be a data point from
  \Table{tab:exponential:radioactive_dice}, where $\tau$ denotes time
...
\solutionpart{subprob:exponential:radioactive_dice_error_analysis}
\Figure{fig:exponential:radioactive_dice_errors} shows the errors of
...
functions.  \Figure{fig:exponential:radioactive_dice_errors} shows
that overall $P(t)$ seems to be better than the other two functions at
modelling the data in \Table{tab:exponential:radioactive_dice}.  This
...
RMSE at approximately $7.890036$, rounded to six decimal places.
\end{solution}
}{}

\begin{table}[!htbp]
\centering
\input{table/11/aluminium.tex}
\caption{%%
...
\item\emph{Aluminium.}
  The experiments from \Exercise{ex:exponential:MnM_experiment} and
  \Problem{prob:exponential:radioactive_dice} can provide you a basic
  ...
  \Table{tab:exponential:aluminium_radioactive_decay}) from an actual
  ...
  \item\label{subprob:exponential:aluminium_graph}
    Graph the data in
    \Table{tab:exponential:aluminium_radioactive_decay} as a scatter
    plot.  What do you notice?

  \item\label{subprob:exponential:aluminium_mean_decay_factor}
    Assume that the data of
    \Table{tab:exponential:aluminium_radioactive_decay} follow the
    exponential decay model of
    \Theorem{thm:exponential:exponential_decay}.  Starting from time
    ...
    \Table{tab:exponential:aluminium_radioactive_decay}.  Use the
    ...
    \Table{tab:exponential:aluminium_radioactive_decay} were taken
    approximately after every $30$ seconds.  Graph the formula
    $M(\tau)$ together with the graph
    from \Part{subprob:exponential:aluminium_graph}.  What do you
    notice?
  ...
  \item\label{subprob:exponential:aluminium_exponential_dice}
    ...
    $t = 0$.  Let $r \geq 0$ be the decay rate as a decimal number.
    The number $P(t)$ of remaining radioactive isotopes after $t$
    units of time can be written as
    %%
    \begin{equation}
    ...
    \end{equation}
    %%
    From \Table{tab:exponential:aluminium_radioactive_decay} you know
    ...
    in \Part{subprob:exponential:aluminium_mean_decay_factor}, note
    that $\tau = t / 30$ for each time value $t$ in
    \Table{tab:exponential:aluminium_radioactive_decay}.  Graph
    $D(\tau)$ together with the graph
    from \Part{subprob:exponential:aluminium_mean_decay_factor}.  What
    do you notice?

  \item\label{subprob:exponential:aluminium_mean_decay_rate}
    Suppose that in
    \Equation{eqn:exponential:aluminium_exponential_model_e} the decay
    ...
    from \Part{subprob:exponential:aluminium_exponential_dice} and
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:aluminium_graph}
\Figure{fig:exponential:aluminium_radioactive_decay} shows a scatter
plot of the data in
\Table{tab:exponential:aluminium_radioactive_decay}.  The figure
clearly shows that the radioactive decay of aluminium follows an
...
\caption{%%
  ...
  \Table{tab:exponential:aluminium_radioactive_decay}.  The blue curve
  is a graph of
  \Equation{eqn:exponential:aluminium_mean_decay_factor}.  The green
  curve is a graph of
  \Equation{eqn:exponential:aluminium_dice_probability}.  The red
  curve is a graph of
  \Equation{eqn:exponential:aluminium_mean_decay_rate}.
}
\label{fig:exponential:aluminium_radioactive_decay}
\end{figure}

\solutionpart{subprob:exponential:aluminium_mean_decay_factor}
...
\Theorem{thm:exponential:exponential_decay}, the number $M(\tau)$ of
...
\end{equation}
%%
\Figure{fig:exponential:aluminium_radioactive_decay} shows a graph of
\Equation{eqn:exponential:aluminium_mean_decay_factor} together with a
...
\solutionpart{subprob:exponential:aluminium_exponential_dice}
...
\Figure{fig:exponential:aluminium_radioactive_decay}.  The figure
...
\solutionpart{subprob:exponential:aluminium_mean_decay_rate}
Assume that the experimental data follow the exponential
...
\Figure{fig:exponential:aluminium_error_analysis}, where the error
...
\Table{tab:exponential:aluminium_radioactive_decay}.
\end{solution}
}{}

\begin{table}[!htbp]
\centering
\input{table/11/vapour-pressure.tex}
\caption{%%
...
\item\emph{Vapour pressure of water.}
  ...
  \Table{tab:exponential:water_vapour_pressure} shows the vapour
  pressures of water between the temperatures of $\degreec{0}$ and
  $\degreec{100}$.
  %%
  \begin{packedenum}
  \item\label{subprob:exponential:water_mean_growth_factor}
    Graph the data in \Table{tab:exponential:water_vapour_pressure} as
    a scatter plot.  Assume that the data in
    \Table{tab:exponential:water_vapour_pressure} follow an
  ...
  \item\label{subprob:exponential:water_Wikipedia_formula}
    ...
    \Table{tab:exponential:water_vapour_pressure}:\footnote{
    ...
    \Equation{eqn:exponential:vapour_pressure_Wikipedia_formula}
    together with the graph
    from \Part{subprob:exponential:water_mean_growth_factor}.  What do
    you notice?
  ...
  \item\label{subprob:exponential:vapour_pressure_Tetens}
    ...
    \Equation{eqn:exponential:vapour_pressure_Tetens} together with
    the graph from \Part{subprob:exponential:water_Wikipedia_formula}.
    What do you notice?
  ...
  \item\label{subprob:exponential:vapour_pressure_Buck}
    ...
    Buck \Equation{eqn:exponential:vapour_pressure_Buck} together with
    the graph from \Part{subprob:exponential:vapour_pressure_Tetens}.
    What do you notice?

  \item\label{subprob:exponential:vapour_pressure_RMS_error}
    For each observed pressure number in
    \Table{tab:exponential:water_vapour_pressure}, calculate the
    errors that result from using each of
    equations~\eqref{eqn:exponential:vapour_pressure_Wikipedia_formula},
    \eqref{eqn:exponential:vapour_pressure_Tetens},
    and~\eqref{eqn:exponential:vapour_pressure_Buck} to predict the
    vapour pressure at the given temperature.  Graph the temperatures
    versus the errors.  Use the graph to identify which of the above
    three equations is better at modelling vapour pressure for which
    range of temperatures.  Use the root mean square error to explain
    which one of the three equations is better at modelling the data
    in \Table{tab:exponential:water_vapour_pressure}.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:water_mean_growth_factor}
...
%%
\begin{equation}
\label{eqn:exponential:water_mean_growth_factor}
f(t)
=
4.579 \times 1.0525^t.
\end{equation}
%%
The latter equation is graphed in
\Figure{fig:exponential:water_vapour_pressure} together with the data
from \Table{tab:exponential:water_vapour_pressure}.  The figure shows
that \Equation{eqn:exponential:water_mean_growth_factor} tends to
...
\caption{%%
  ...
  \Table{tab:exponential:water_vapour_pressure}.  The green curve is a
  graph of \Equation{eqn:exponential:water_mean_growth_factor}.  The
  dashed curve is a graph of
  \Equation{eqn:exponential:vapour_pressure_Wikipedia_formula}.  The
  blue curve is a graph of the Tetens
  \Equation{eqn:exponential:vapour_pressure_Tetens}.  The graph of the
  Buck \Equation{eqn:exponential:vapour_pressure_Buck} is not shown
...
\solutionpart{subprob:exponential:water_Wikipedia_formula}
\solutionpart{subprob:exponential:vapour_pressure_Tetens}
\solutionpart{subprob:exponential:vapour_pressure_Buck}
\Figure{fig:exponential:water_vapour_pressure} compares the graphs of
\Equations{eqn:exponential:vapour_pressure_Wikipedia_formula}{eqn:exponential:water_mean_growth_factor}
and the Tetens \Equation{eqn:exponential:vapour_pressure_Tetens}.
Note that the graph for the Buck
\Equation{eqn:exponential:vapour_pressure_Buck} is not shown because
it overlaps with the graphs of
\Equation{eqn:exponential:vapour_pressure_Wikipedia_formula} and the
Teten equation.  The figure shows that
\Equation{eqn:exponential:vapour_pressure_Wikipedia_formula} is better
suited to modelling the given data set than is
\Equation{eqn:exponential:water_mean_growth_factor}.  The Tetens and
...
\caption{%%
  ...
  Tetens \Equation{eqn:exponential:vapour_pressure_Tetens}, and the
  Buck \Equation{eqn:exponential:vapour_pressure_Buck}.
}
\label{fig:exponential:vapour_pressure_errors}
\end{figure}

\solutionpart{subprob:exponential:vapour_pressure_RMS_error}
\Figure{fig:exponential:vapour_pressure_errors} compares the errors of
the exponential
model~\eqref{eqn:exponential:vapour_pressure_Wikipedia_formula}, the
Tetens \Equation{eqn:exponential:vapour_pressure_Tetens}, and the Buck
\Equation{eqn:exponential:vapour_pressure_Buck}.  The figure shows
that the exponential
\Equation{eqn:exponential:vapour_pressure_Wikipedia_formula} can be
...
\Equation{eqn:exponential:vapour_pressure_Tetens} can be used to
...
prediction of the Buck \Equation{eqn:exponential:vapour_pressure_Buck}
...
\Table{tab:exponential:water_vapour_pressure}.
\end{solution}
}{}

\begin{table}[!htbp]
\centering
\input{table/11/yeast.tex}
\caption{%%
...
\item\emph{Logistic function.}
  ...
  in \Section{sec:exponential:exponential_growth} can produce results
  ...
  \item\label{subprob:exponential:yeast_data_mean_growth_factor}
    \Table{tab:exponential:yeast} shows the population numbers of an
    ...
    the experimental data in \Table{tab:exponential:yeast}.  On the
    same set of axes, graph your formula $Y(t)$.  What do you notice?

  \item\label{subprob:exponential:yeast_quintic_polynomial}
    Suppose you model the data in \Table{tab:exponential:yeast} as a
    ...
    \Equation{eqn:exponential:yeast_quintic_polynomial}, your formula
    from \Part{subprob:exponential:yeast_data_mean_growth_factor}, and
    the data from \Table{tab:exponential:yeast}.  What do you notice?

  \item\label{subprob:exponential:yeast_logistic}
    In~1927, Raymond Pearl used the logistic
    \Equation{eqn:exponential:logistic_equation} to model the data in
    \Table{tab:exponential:yeast}.\footnote{
      See the following paper for further details:
    ...
    %%
    \begin{equation}
    \label{eqn:exponential:yeast_Pearl_logistic_equation}
    P(t)
    =
    \frac{
      665
    ...
    \end{equation}
    %%
    Graph \Equation{eqn:exponential:yeast_Pearl_logistic_equation} on
    the same set of axes as the graph
    in \Part{subprob:exponential:yeast_quintic_polynomial}.  What do
    you notice?  Graph the errors of the functions $Y(t)$, $f(t)$, and
    $P(t)$ on one set of axes.  What do you notice?  Use the root mean
    square error to help you explain which one of the above three
    equations is more suitable at modelling the data in
    \Table{tab:exponential:yeast}.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:yeast_data_mean_growth_factor}
\Table{tab:exponential:yeast_hourly_growth_factors} shows the hourly
growth factors of the experimental colony of yeast.  Using the hourly
growth factors, the mean growth factor is calculated to be $1.2937$,
rounded to four decimal places.  Suppose that the yeast population
grew according to an exponential model.  Let $a = 9.6$ yeast cells be
the initial population and let $b = 1.2937$ be the growth factor.  If
$Y(t)$ represents the population size after $t$ hours, then the
population size can be written as
%%
\begin{equation}
\label{eqn:exponential:yeast_exponential_model}
Y(t)
=
9.6 \times 1.2937^t.
\end{equation}
%%
\Figure{fig:exponential:yeast_data_versus_models} shows a graph of the
exponential model~\eqref{eqn:exponential:yeast_exponential_model}
fitted to the experimental data given in
\Table{tab:exponential:yeast}.  The figure clearly shows that an
exponential growth model is not appropriate for the data set under
consideration.

\begin{table}[!htbp]
\centering
\input{table/11/yeast-growth-factor.tex}
\caption{%%
  ...
  experimental data are from \Table{tab:exponential:yeast}.
}
\label{tab:exponential:yeast_hourly_growth_factors}
\end{table}
...
\caption{%%
  ...
  experimental data are from \Table{tab:exponential:yeast}.  The
...
\solutionpart{subprob:exponential:yeast_quintic_polynomial}
\Figure{fig:exponential:yeast_data_versus_models} shows a comparison
of the experimental data given in \Table{tab:exponential:yeast}, the
...
\solutionpart{subprob:exponential:yeast_logistic}
\Figure{fig:exponential:yeast_data_versus_models} shows a comparison
of three functions $Y(t)$, $f(t)$, and $P(t)$ for modelling the
experimental data given in \Table{tab:exponential:yeast}.  Of the
...
\Figure{fig:exponential:yeast_error_analysis}.  The figure shows that
...
functions at modelling the data in \Table{tab:exponential:yeast}.
...
\item\emph{Geometric distribution.}
  ...
  \item\label{subprob:exponential:coin_simulation_fair_coin}
    ...
    \end{center}
    %%
    The purpose of the function \code{toss\_coin()} is to simulate the
    ...
    function should return \code{True} if the result of the coin toss
  ...
  \item\label{subprob:exponential:coin_simulation_time_to_heads}
    ...
    %%
    \begin{center}
    \code{time\_to\_heads()}
    \end{center}
    %%
    The function \code{time\_to\_heads()} should simulate the number
    ...
    \code{toss\_coin()} until heads occurs for the first time.  The
    function \code{time\_to\_heads()} should return the number of coin
  ...
  \item\label{subprob:exponential:coin_simulation_main}
    ...
    \end{center}
    %%
    The purpose of the function \code{main()} is to control your coin
    ...
    \code{time\_to\_heads()}, you perform one round of the
    ...
    end of the simulation, \code{main()} should write to a file the
  ...
  \item\label{subprob:exponential:coin_simulation_estimate_probability}
    ...
    in \Part{subprob:exponential:coin_simulation_frequency_distribution}.
    ...
    $\phat$ and
    \Equation{eqn:exponential:coin_simulation_geometric_distribution}
    to calculate the predicted probability $\Pr(k)$.  Organise your
    results as another column in the table
    from \Part{subprob:exponential:coin_simulation_frequency_distribution}.
    Plot the predicted probabilities together with the graph
    from \Part{subprob:exponential:coin_simulation_frequency_distribution}.

  \item\label{subprob:exponential:coin_simulation_mean_tosses}
    Let $\mu$ be the mean number of coin tosses required for the first
    occurrence of heads.  If $\phat$ is as in
    \Equation{eqn:exponential:coin_simulation_estimate_p}, explain why
    the mean can be written as $\mu = 1 / \phat$.  Use the latter
    expression to calculate~(for your experimental data) the mean
    number of coin tosses required in order to get heads for the first
    time.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:exponential:coin_simulation_frequency_distribution}
See \Table{tab:exponential:coin_simulation_frequency_distribution} for
a summary of the experimental data from one run of the coin tossing
simulation.  See \Figure{fig:exponential:coin_simulation_probability}
for the corresponding probability plot.  As the figure shows, the
probability distribution of $k$ coin tosses to the first heads seems
to follow an exponential decay model.

\begin{table}[!htbp]
\centering
\input{table/11/coin.tex}
\caption{%%
  The number $k$ of coin tosses to the first heads versus the
  probability of $k$.  The values in the ``Count'' column should sum
  to $10,000$.  The predicted probability values in the
  ``Pred. prob.'' column were calculated using
  \Equation{eqn:exponential:coin_simulation_predicted_probability}.
}
\label{tab:exponential:coin_simulation_frequency_distribution}
\end{table}

\begin{figure}[!htbp]
\centering
\includegraphics[scale=1.1]{image/11/coin.pdf}
\caption{%%
  The probability distribution of $k$ coin tosses to the first heads.
  The black dots show a graph of the data from
  \Table{tab:exponential:coin_simulation_frequency_distribution}.  The
  red curve shows a graph of
  \Equation{eqn:exponential:coin_simulation_predicted_probability}.
}
\label{fig:exponential:coin_simulation_probability}
\end{figure}

\solutionpart{subprob:exponential:coin_simulation_estimate_probability}
...
\Table{tab:exponential:coin_simulation_frequency_distribution}, you
have
%%
\begin{align*}
\sum_{k=1}^{15} k c_k
...
column in
\Table{tab:exponential:coin_simulation_frequency_distribution}.  In
other words, heads occurred $10,000$ times during the simulation.
Using \Equation{eqn:exponential:coin_simulation_estimate_p}, the
...
%%
which is graphed in
\Figure{fig:exponential:coin_simulation_probability}.  See
\Table{tab:exponential:coin_simulation_frequency_distribution} for
values of the predicted probability calculated by using the estimate
$\phat$.

\solutionpart{subprob:exponential:coin_simulation_mean_tosses}
...
\Equation{eqn:exponential:coin_simulation_estimate_p}.  For the
