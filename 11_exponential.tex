...
\ifbool{showSolution}{
\begin{solution}
In \Table{tab:Australian_population_2017}, the year $2017$ corresponds
to $t = 0$ years since $2017$ so according to
...
\Figure{fig:carbon14_decay} shows a graph of the amount of carbon-$14$
...
number $A(t)$ of isotopes of carbon-$14$ remaining after
...
after $t$ units of time, use \Theorem{thm:exponential_decay} to write
$A(t) = 100 \times 0.5^t$.  Let's use the latter formula to verify the
results from \Part{subex:fermium250_30minutes_60minutes}.  The time of
...
\item\label{subex:MnM_probability}
  ...
  with the graph from \Part{subex:MnM_data}.  What do you notice?  Use
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subex:MnM_data}
\Table{tab:MnM_data} shows the data for one run of the M\&M's
experiment.  \Figure{fig:MnM_graph} shows a graph of the number of
remaining M\&M's after $t$ minutes.  The figure shows that the
``decay'' of the M\&M's seems to follow a model of exponential decay.
...
\caption{%%
  ...
  black dots show the experimental data from \Table{tab:MnM_data}.
  The blue curve is a graph of \Equation{eqn:MnM_probability_model}.
}
\label{fig:MnM_graph}
\end{figure}

\solutionpart{subex:MnM_decay_factor}
If the ``decay'' of the M\&M's were to follow an exponential decay
...
\solutionpart{subex:MnM_probability}
...
\end{equation}
%%
\Figure{fig:MnM_graph} shows a graph of
\Equation{eqn:MnM_probability_model}.  The figure shows that the
...
RMSE indicates that \Equation{eqn:MnM_probability_model} is fairly
...
\item\label{prob:duckweeds}
  ...
  \item\label{subprob:duckweeds_growth_factor_estimate}
    ...
    from \Part{subprob:duckweeds_growth_factor}, you can estimate the
  ...
  \item\label{subprob:duckweeds_growth_factor_exponential}
    ...
    graph of \Equation{eqn:frond_numbers_Ashby_formula}.

  \item\label{subprob:duckweeds_error_analysis}
    For each observed frond number in
    \Table{tab:duckweed_frond12_missing}, calculate the errors that
    result from using your formula
    from \Part{subprob:duckweeds_growth_factor_estimate} to predict
    the frond number.  Repeat the error calculation for
    \Equation{eqn:frond_numbers_Ashby_formula}.  Produce a graph of
    the day number versus the errors.  What do you notice?  By using
    the root mean square error, explain which of
    \Equation{eqn:frond_numbers_Ashby_formula} and your formula
...
\caption{%%
  ...
  \Equation{eqn:fronds_formula_average_growth_factor} and Ashby's
  formula~\eqref{eqn:frond_numbers_Ashby_formula} to predict the
  number of fronds during a given day.  Let $\tuple{i}{d_i}$ be a data
  point from \Table{tab:duckweed_frond12_missing}, where $d_i$ denotes
  the number of fronds observed during day $i$.  The prediction of
  \Equation{eqn:fronds_formula_average_growth_factor} for day $i$ is
  ...
  \Equation{eqn:fronds_formula_average_growth_factor} are the errors
...
\solutionpart{subprob:duckweeds_error_analysis}
\Figure{fig:frond_error_analysis} compares the errors that result from
using \Equation{eqn:fronds_formula_average_growth_factor} and Ashby's
...
of \Equation{eqn:fronds_formula_average_growth_factor} is $4.8256$,
...
conclude that \Equation{eqn:fronds_formula_average_growth_factor} is
...
\item\label{prob:radioactive_dice}
  ...
  \item\label{subprob:radioactive_dice_decay_constant}
    ...
    decimal places.\footnote{
      ...
      \Equation{eqn:half_life_related_to_decay_rate}.
    }
    Use your graph
...
\Table{tab:radioactive_dice} shows the data from one run of the above
experiment with $1000$ ``radioactive'' dice~(the dice were not
actually radioactive).  See \Figure{fig:radioactive_dice} for a
scatter plot of the experimental data in \Table{tab:radioactive_dice}.
The table shows that after the elapse of $42$ minutes you have zero
dice remaining.  In other words, in this particular run of the
experiment $42$ minutes were required in order for all the $1000$
``radioactive'' dice to ``decay''.
...
\item\emph{Aluminium.}
  The experiments from \Exercise{ex:MnM_experiment} and
  \Problem{prob:radioactive_dice} can provide you a basic
  ...
  \Table{tab:aluminium_radioactive_decay}) from an actual experiment
  ...
  \item\label{subprob:aluminium_graph}
    Graph the data in \Table{tab:aluminium_radioactive_decay} as a
    scatter plot.  What do you notice?

  \item\label{subprob:aluminium_mean_decay_factor}
    Assume that the data of \Table{tab:aluminium_radioactive_decay}
    follow the exponential decay model of
    \Theorem{thm:exponential_decay}.  Starting from time $t = 30$
    ...
    \Table{tab:aluminium_radioactive_decay}.  Use the estimate of $b$
    ...
    \Table{tab:aluminium_radioactive_decay} were taken approximately
    after every $30$ seconds.  Graph the formula $M(\tau)$ together
    with the graph from \Part{subprob:aluminium_graph}.  What do you
    notice?
  ...
  \item\label{subprob:aluminium_exponential_dice}
    ...
    \end{equation}
    %%
    From \Table{tab:aluminium_radioactive_decay} you know that
    ...
    in \Part{subprob:aluminium_mean_decay_factor}, note that
    $\tau = t / 30$ for each time value $t$ in
    \Table{tab:aluminium_radioactive_decay}.  Graph $D(\tau)$ together
    with the graph from \Part{subprob:aluminium_mean_decay_factor}.
    What do you notice?

  \item\label{subprob:aluminium_mean_decay_rate}
    Suppose that in \Equation{eqn:aluminium_exponential_model_e} the
    ...
    from \Part{subprob:aluminium_exponential_dice} and describe what
...
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:aluminium_graph}
\Figure{fig:aluminium_radioactive_decay} shows a scatter plot of the
data in \Table{tab:aluminium_radioactive_decay}.  The figure clearly
shows that the radioactive decay of aluminium follows an exponential
decay model.
...
\caption{%%
  ...
  the experimental data from \Table{tab:aluminium_radioactive_decay}.
  The blue curve is a graph of
  \Equation{eqn:aluminium_mean_decay_factor}.  The green curve is a
  graph of \Equation{eqn:aluminium_dice_probability}.  The red curve
  is a graph of \Equation{eqn:aluminium_mean_decay_rate}.
}
\label{fig:aluminium_radioactive_decay}
\end{figure}

\solutionpart{subprob:aluminium_mean_decay_factor}
...
\Theorem{thm:exponential_decay}, the number $M(\tau)$ of remaining
...
\end{equation}
%%
\Figure{fig:aluminium_radioactive_decay} shows a graph of
\Equation{eqn:aluminium_mean_decay_factor} together with a scatter
...
\solutionpart{subprob:aluminium_exponential_dice}
...
\Figure{fig:aluminium_radioactive_decay}.  The figure shows that
...
\solutionpart{subprob:aluminium_mean_decay_rate}
Assume that the experimental data follow the exponential
...
\end{equation}
%%
\Figure{fig:aluminium_radioactive_decay} shows a graph of $R(t)$.  The
...
\Figure{fig:aluminium_error_analysis}, where the error graph for
...
\Table{tab:aluminium_radioactive_decay}.
\end{solution}
}{}
...
\item\emph{Vapour pressure of water.}
  ...
  \Table{tab:water_vapour_pressure} shows the vapour pressures of
  ...
  \item\label{subprob:vapour_pressure_RMS_error}
    ...
    \Table{tab:water_vapour_pressure}, calculate the errors that
    result from using each of
    ...
    the errors.  Use the graph to identify which of the above three
    equations is better at modelling vapour pressure for which range
    of temperatures.  Use the root mean square error to explain which
    one of the three equations is better at modelling the data in
...
\Figure{fig:vapour_pressure_errors} compares the errors of the
exponential model~\eqref{eqn:vapour_pressure_Wikipedia_formula}, the
Tetens \Equation{eqn:vapour_pressure_Tetens}, and the Buck
...
\item\emph{Logistic function.}
  ...
  \item\label{subprob:yeast_logistic}
    ...
    What do you notice?  Graph the errors of the functions $Y(t)$,
    $f(t)$, and $P(t)$ on one set of axes.  What do you notice?  Use
    the root mean square error to help you explain which one of the
    above three equations is more suitable at modelling the data in
    \Table{tab:yeast}.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
...
data given in \Table{tab:yeast}.  Of the three functions, the logistic
...
\Figure{fig:yeast_error_analysis}.  The figure shows that the function
...
modelling the data in \Table{tab:yeast}.  This conclusion is supported
...
\item\emph{Geometric distribution.}
  ...
  \item\label{subprob:coin_simulation_estimate_probability}
    ...
    in \Part{subprob:coin_simulation_frequency_distribution}.  Note
    ...
    $\phat$ and
    \Equation{eqn:coin_simulation_geometric_distribution}
    to calculate the predicted probability $\Pr(k)$.  Organise your
    results as another column in the table
    from \Part{subprob:coin_simulation_frequency_distribution}.
    Plot the predicted probabilities together with the graph
    from \Part{subprob:coin_simulation_frequency_distribution}.

  \item\label{subprob:coin_simulation_mean_tosses}
    Let $\mu$ be the mean number of coin tosses required for the first
    occurrence of heads.  If $\phat$ is as in
    \Equation{eqn:coin_simulation_estimate_p}, explain why the mean
    can be written as $\mu = 1 / \phat$.  Use the latter expression to
    calculate~(for your experimental data) the mean number of coin
    tosses required in order to get heads for the first time.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
\solutionpart{subprob:coin_simulation_frequency_distribution}
See \Table{tab:coin_simulation_frequency_distribution} for a summary
of the experimental data from one run of the coin tossing simulation.
See \Figure{fig:coin_simulation_probability} for the corresponding
probability plot.  As the figure shows, the probability distribution
of $k$ coin tosses to the first heads seems to follow an exponential
decay model.
...
\caption{%%
  ...
  \Equation{eqn:coin_simulation_predicted_probability}.
}
\label{tab:coin_simulation_frequency_distribution}
\end{table}
...
\caption{%%
  ...
  \Table{tab:coin_simulation_frequency_distribution}.  The red curve
  shows a graph of
  \Equation{eqn:coin_simulation_predicted_probability}.
}
\label{fig:coin_simulation_probability}
\end{figure}

\solutionpart{subprob:coin_simulation_estimate_probability}
...
\]
where $n$ is as
in \Part{subprob:coin_simulation_frequency_distribution}.  Using the
experimental data in
\Table{tab:coin_simulation_frequency_distribution}, you have
%%
\begin{align*}
\sum_{k=1}^{15} k c_k
&=
...
column in \Table{tab:coin_simulation_frequency_distribution}.  In
other words, heads occurred $10,000$ times during the simulation.
Using \Equation{eqn:coin_simulation_estimate_p}, the probability that
...
\end{equation}
%%
which is graphed in \Figure{fig:coin_simulation_probability}.
See \Table{tab:coin_simulation_frequency_distribution} for values of
the predicted probability calculated by using the estimate $\phat$.

\solutionpart{subprob:coin_simulation_mean_tosses}
...
\frac{h}{N}
\]
which is the same as in \Equation{eqn:coin_simulation_estimate_p}.
For the experimental data in
\Table{tab:coin_simulation_frequency_distribution}, the mean $\mu$ of
