...
\caption{%%
  A unit circle drawn inside a square.  The circle touches the square
  at the given four points.
}
\label{fig:square_inscribed_circle}
\end{figure}

\item A unit circle is inscribed within a square as shown in
  \Figure{fig:square_inscribed_circle}.  The circle touches the square
  at exactly the four points shown in the figure.  Calculate the area
  of the shaded region in \Figure{fig:square_inscribed_circle}.
\ifbool{showSolution}{
\begin{solution}
In \Figure{fig:square_inscribed_circle}, the area of the shaded region
is the difference between the area of the square and the area of the
circle.  A unit circle has a radius of $r = 1$ unit so the circle has
an area of $\pi r^2 = \pi$ units.  Each side of the square has a
length of $2$ units so the area of the square is $2^2 = 4$ units.
Therefore the area $A$ of the shaded region is obtained by subtracting
the area of the circle from the area of the square.  That is,
$A = 4 - \pi$ units.
\end{solution}
}{}
\end{problem}
