...
\ifbool{showSolution}{
\begin{solution}
First, you simplify the sums inside the parentheses and then multiply
the results together.  This will result in
%%
\begin{align*}
(1 + 2) \times (3 + 4)
&=
...
\end{align*}
%%
On the other hand, use the distributive laws to simplify the
...
\ifbool{showSolution}{
\begin{solution}
The expression $a - (-b) - c$ can be simplified as:
%%
\begin{align*}
a - (-b) - c
&=
...
\ifbool{showSolution}{
\begin{solution}
The expression $2 + 7 \times 3 - 1$ can be simplified as:
%%
\begin{align*}
2 + 7 \times 3 - 1
&=
...
\begin{exercise}
Let $p$ denote Patrick's age, $m$ be Marcus' age, $M$ your mum's age,
and $d$ your dad's age.  The sum of all these ages is
\[
p + m + M + d.
\]
...
\ifbool{showSolution}{
\begin{solution}
You can use the associative laws to write
%%
\begin{align*}
p + m + M + d
&=
...
\end{align*}
%%
This indicates that you want to first simplify the sums within the
parentheses.  Use your result to then simplify the whole expression.
\end{solution}
}{}
...
\begin{problem}
\item Classify the following as positive integer, negative integer,
  integer, or not an integer.
  %%
  \begin{packedenum}
  \item $\sqrt{2}$\ifbool{showSolution}{, not an integer}{}

  \item $2.0$\ifbool{showSolution}{, positive integer because $2.0 = 2$}{}

  \item $0$\ifbool{showSolution}{, an integer}{}

  \item $-1$\ifbool{showSolution}{, negative integer}{}

  \item orange\ifbool{showSolution}{, not an integer}{}

  \item $-2.1$\ifbool{showSolution}{, not an integer}{}
  \end{packedenum}
...
\ifbool{showSolution}{
  ...
  number of toes on your feet, the number of houses in Melbourne.
  \end{solution}
}{}
...
\ifbool{showSolution}{
  ...
  \end{align*}
  %%
  On the other hand, you can use the distributive laws to write:
  %%
  \begin{align*}
  2(4 + 1) + 3(5 + 2)
  &=
...
\ifbool{showSolution}{
  \begin{solution}
  This is an incorrect use of the distributive laws.  The
  simplification should be:
  %%
  \begin{align*}
  2 \times (1 + 3)
  &=
  (2 \times 1) + (2 \times 3) \\[4pt]
  &=
  2 + 6 \\[4pt]
  &=
  8.
  \end{align*}
  %%
  To check your answer, use the distributive laws to first simplify
...
\item Let $x$ be any even integer and $y$ any odd integer.  Provide
  two explanations as to why the product $xy = x \times y$ is an even
  integer.
\ifbool{showSolution}{
\begin{solution}
...
\end{align*}
%%
The number $2 (2kr + k)$ is even because the product of $2$ and any
integer is even.  Therefore $xy$ is even.

As a check, note that since $x = 2k$ then you can write
%%
\begin{align*}
xy
&=
...
\end{align*}
%%
which is even.  Therefore the product $xy$ is an even integer.
\end{solution}
}{}

\item Let $n$ be any even integer.
  %%
  \begin{packedenum}
  \item\label{subprob:integer_squared_is_even}
    Explain why the square number $n^2 = n \times n$ is also an even
    integer.

  \item\label{subprob:integer_squared_is_even_another_proof}
    Provide another explanation of why $n^2$ is even.
  \end{packedenum}
\ifbool{showSolution}{
  \begin{solution}
  \solutionpart{subprob:integer_squared_is_even}
  Since $n$ is an even integer, then $n^2 = n \times n$ is the product
  of two even integers.  Use
  \Theorem{thm:product_of_two_evens_is_even} to conclude that the
  square number $n^2$ is also even.

  \solutionpart{subprob:integer_squared_is_even_another_proof}
  Since $n$ is even you can write it as $n = 2k$, where $k$ is another
  integer.  Use the associative laws to write $n^2$ as
  %%
  \begin{align*}
  n^2
  &=
