...
in~\eqref{eqn:Hong_Kong_teenagers_heights} have a mean of $173.8$
centimetres.
...
of $30$ people.\footnote{
  The data are from the following website:
  \url{http://web.archive.org/web/20180428211320/http://people.sc.fsu.edu/~jburkardt/datasets/regression/regression.html},
  accessed 2018-04-28.
}
%%
\begin{packedenum}
...
\item Read the following article by Brian Hayes:
  \emph{Gauss' day of reckoning}.\footnote{
    Available at
    \url{http://web.archive.org/web/20180509025720/http://www.bit-player.org/wp-content/extras/bph-publications/AmSci-2006-05-Hayes-Gauss.pdf},
    accessed 2018-05-09.
  }
...
\item This problem will help you to understand the idea of the
  ...
  \item\label{subprob:Basel_problem_sum_first_1000}
    ...
    since \Part{subprob:Basel_problem_sum_first_5} as a table that has
    two columns and as many rows as necessary.  The first row has the
    column headings.  Produce a scatter plot of the data in your
    table.
...
\item Let's further explore the topic of probability.  However, this
  time you will consider the example of flipping a coin, for example
  an Australian $\cent{20}$ coin.  Let's assume that by \emph{heads}
  you mean the side of the coin that has the profile of the head of
  the Queen of England.  By \emph{tails} you mean the side of the coin
  that has the number with the monetary value of the coin~(e.g.~the
  number $20$ for twenty cents).
  %%
  \begin{packedenum}
...
\begin{figure}[!htbp]
\centering
\includegraphics[scale=1.1]{image/07/coin-flip-scatterplot.pdf}
\caption{%%
