...
$g(x) = a \cos(cx) + b$.  The amount of time required for the graphs
of $f(x)$ and $g(x)$ to complete one cycle is called the \emph{period}
of the functions.  The period of $f(x)$ and $g(x)$ is defined as
$2\pi / \absoluteValue{c}$.
\end{definition}
...
\textbf{Frequency.}
Let $\triple{a}{b}{c}$ be real constants and let $x$ be a real
variable.  Consider the functions
\[
f(x)
=
a \sin(cx) + b
...
\begin{example}
Let $c$ be a real constant and consider the function
...
Up to this point, you have learnt about how to stretch the graph of a
sine or cosine function along the horizontal and vertical axes.  You
have also learnt about how to vertically shift the graph of a sine or
cosine function.  What remains to be discussed is how to shift the
graph of a sine or cosine function along the horizontal axis.
...
the sine function $f(x) = \sin x$ and the cosine function
$g(x) = \cos x$.  Note that you can obtain the graph of the cosine
function $g(x)$ by shifting the graph of the sine function $f(x)$ to
the left.  Looking at the graphs in
