...
the object.  \Figure{fig:trigonometric:spring_frequency_vary_mass}
shows the effect on the object's frequency as the mass increases.
With the spring constant being fixed, as the mass of the object
increases the frequency decreases.  The reason is that as the object
becomes heavier the spring is stretched longer, so the object requires
more time to oscillate between its minimum and maximum heights.  Hence
the number of cycles per second becomes smaller and smaller, which
means more and more time is required to complete one cycle.
