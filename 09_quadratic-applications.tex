...
\caption{%%
  ...
  reserved for the orbital periods~(exact, Kepler's equation,
  ...
  $365.2$ Earth days.  The third column contains the exact orbital
...
\item In the Solar System, the distance from any planet to the Sun is
  ...
  \item\label{subprob:planetary_orbit_comparison}
    ...
    functions is better?  One way to help you decide is to see which
    of the two functions gives you the lowest
    \emph{root mean square error}~or RMS error.  Here, the word
    \emph{error} has the meaning: how far away is the predicted value
    from the observed value.  In this case, an \emph{observed value}
    is an exact value of an orbital period as given in
    \Table{tab:planet_distance_to_Sun} and a \emph{predicted value} is
    a value of the orbital period as calculated by some function.  The
    better is a function at modelling the orbital periods given in
    \Table{tab:planet_distance_to_Sun}, the lower should be the RMS
    ...
    $y_i - \widehat{y_i}$, hence the squared error is
    $(y_i - \widehat{y_i})^2$.  Calculate the mean of all the squared
    errors and take the square root of the mean.  The result is the
    RMS error, which can be written as the expression
    %%
    \begin{align*}
    \text{RMS error}
    &=
    ...
    \end{align*}
    %%
    By using the RMS error, explain which of
    \Equations{eqn:Earth_days_orbit_approximation}{eqn:Earth_days_orbit_Kepler}
    is better at modelling the orbital periods of the planets listed
    in \Table{tab:planet_distance_to_Sun}.
  \end{packedenum}
\ifbool{showSolution}{
\begin{solution}
...
\Table{tab:planet_orbital_periods} shows the completed version of
\Table{tab:planet_distance_to_Sun} with all missing entries filled
in.  The table also shows the squared errors of the predicted values
from
\Equations{eqn:Earth_days_orbit_approximation}{eqn:Earth_days_orbit_Kepler}.
...
rounded to four decimal places.  Therefore you conclude that Kepler's
equation provides a better model of the orbital periods of the given
planets than does the quadratic function.

\begin{table}[!htbp]
\centering
\input{table/09/planets-complete.tex}
\caption{%%
  The orbital period of a planet as a function of its distance~(in
...
\label{tab:planet_orbital_periods}
\end{table}
