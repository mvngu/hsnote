...
\begin{problem}
\item Suppose $a \geq 0$ is a real number.  Let $A$ be the distance
...
\ifbool{showSolution}{
  ...
  Therefore each of the points is the same distance from the origin.
  On the other hand, you can use
  \Theorem{thm:distance_between_two_points} to calculate the
  distances.  The distance $A$ is
  \[
  A
  =
  \sqrt{
...
\item\label{prob:distance_from_point_to_itself_is_zero}
  If $A = \tuple{a}{b}$ is any point in the Cartesian coordinate
...
\item Let $a$ be a real number.  Explain why the distance from the
  point $\tuple{a}{0}$ to the origin is the absolute value
  $\absoluteValue{a}$.
\ifbool{showSolution}{
  \begin{solution}
  You have three cases: (1)~$a > 0$, (2)~$a < 0$, and (3)~$a = 0$.  If
  $a > 0$, then the point $\tuple{a}{0}$ has a distance of $a$ to the
  origin.  Note that $\absoluteValue{a} = a$ because you assumed that
  $a > 0$.  If $a < 0$, then $\tuple{a}{0}$ has a distance of
  $\absoluteValue{a}$ to the origin.  If $a = 0$, then from
  \Problem{prob:distance_from_point_to_itself_is_zero} you know that
  the distance from $\tuple{0}{0}$ to itself is zero.  Note that
  $\absoluteValue{0} = 0$.  In all of the above three cases, you have
  a distance of $\absoluteValue{a}$ from $\tuple{a}{0}$ to the origin.
  \end{solution}
}{}
...
\item Let $\pair{a}{b} > 0$ be real numbers such that $a \leq b$.  Let
  ...
  segment $L$ is the line that starts from $A$ and ends at $B$.
  Suppose you want to break up the segment $L$ into four pieces of
  equal length.  Determine the start and end coordinates of each of
  the four pieces.
\ifbool{showSolution}{
  \begin{solution}
...
\item Let $a > 0$ be a real number.  An ant starts at the point
  ...
  straight to the point $\tuple{a}{a}$.  From $\tuple{a}{a}$ the ant
  moves in a straight line to $\tuple{0}{a}$.  The ant then moves in a
  straight line to its starting position.  If the ant travelled a
...
\item Consider the right-angled triangle in
  \Figure{fig:right_angled_triangle}.  Compute the value of the
  hypotenuse.
\ifbool{showSolution}{
  \begin{solution}
