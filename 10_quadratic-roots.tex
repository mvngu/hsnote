...
\Expression{eqn:quadroots:monic_quadratic_function} so you may use the
...
\Theorem{thm:quadroots:monic_quadratic_roots}.
...
\Example{eg:quadroots:completing_square_monic_bminus10_c21} below
...
3.
\]
These are the same roots as those derived
...
The function $f(x)$ can be written in such a way that resembles the
...
the original expression for $f(x)$.
...
rectangles has a width and height of $a$ and $x - a$, respectively,
and therefore an area of $a(x - a)$.  Since the two shaded rectangles
have the same dimensions, then the total area of these rectangles is
$2a(x - a)$.  The square in the lower-right corner has a side length
of $x - a$ and thus an area of $(x - a)^2$.  Then the total area of
...
\item You will derive the quadratic formula via a technique known to
  Hindu mathematicians since around the year {\sc ad}~1025.
  % \footnote{
  %   See the following paper for further details:
  %   \url{http://www.jstor.org/stable/27965986}.
  % }
  Consider a quadratic function $f(x) = ax^2 + bx + c$, where
  $\triple{a}{b}{c}$ are real constants such that $a \neq 0$.
  %%
  \begin{packedenum}
...
The roots of $f(x) = ax^2 - c$ are all values of $x$ such that the
expression $f(x) = 0$ is true.  That is, you want all values of $x$
such that the expression $ax^2 - c = 0$ is true.  In the last
expression, adding $c$ to both sides results in $ax^2 = c$.  Now
divide both sides by $a$ to produce $x^2 = c / a$.  Take the square
root of both sides and you have $x = \pm \sqrt{\frac{c}{a}}$.
...
Let $f(x) = ax^2 + bx + c$.  By using the substitution
$x = y - \frac{b}{2a}$, you can write $f(x)$ as
%%
\begin{align*}
...
Consider the function $f(x) = 3x^2 - 7x + 2$.  Use
\Equation{eqn:quadroots:quadratic_roots:Viete_quadratic_formula} to
write
\[
y
=
