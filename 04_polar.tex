...
trigonometric functions; see \Figure{fig:cosine_sine}.  These
functions can be used to calculate one side of a right-angled triangle
given a known value of $\varphi$.  Consider the unit circle in
\Figure{fig:point_on_unit_circle} and let $\tuple{a}{b}$ be a point on
the circle.  Let $\varphi$ be the angle from the positive half of the
$x$-axis to the point $\tuple{a}{b}$, going anti-clockwise.  Then the
values of $a$ and $b$ can be written in terms of sine and cosine as
%%
\begin{equation}
...
\caption{%%
  ...
  $y = \sin x$~(the red line).  The value of $x$ is in radians.
}
...
can be right-angled, isosceles, or scalene.
