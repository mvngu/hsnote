...
As another example, consider the point $\tuple{0}{1}$.
...
point $\tuple{1}{0}$ you have an angle of $\varphi = 0$ radians
because the point $\tuple{1}{0}$ lies on the $x$-axis.  If the point
$\tuple{1}{0}$ makes a right-triangle, then the height of the triangle
has length $0$, the base of the triangle has length $1$, and the
hypotenuse has length $1$ because the hypotenuse is the radius of the
unit circle.  From the angle of $0$ radians, the side adjacent to the
angle is the base of the right-triangle so you can write
$\cos(0) = 1 / 1$, which simplifies to $\cos(0) = 1$.  The side
opposite the given angle has length $0$.  Since the hypotenuse has
length $1$, you can write $\sin(0) = 0 / 1$, which simplifies to
$\sin(0) = 0$.
\end{solution}
