...
\ifbool{showSolution}{
  ...
  \caption{%%
    ...
    length of the radius might be measured in terms of millimetres,
    centimetres, metres, or something else.  You are not concerned
    about these.  You only know that whatever standard of measurement
    is used, you want a value of one in terms of the scale of
    measurement that was used to measure the radius.  So the phrase
    ``one unit'' can mean: one inch, one centimetre, one kilometre,
    etc.
  }
...
\ifbool{showSolution}{
  ...
  $k \geq 0$ is another integer.  Use
  \Exercise{ex:square_root_2_squared_is_rational} to write
  $(\sqrt{2})^n$ as
  %%
  \begin{align*}
  (\sqrt{2})^n
