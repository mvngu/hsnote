...
\Figure{fig:quadratic_as_rectangle}.
...
\end{align*}
%%
All this means that $f(2) = 2^2 = 4 = (-2)^2 = f(-2)$.  Finally, if
$x = 3$ or $x = -3$ then you can write
%%
\begin{align*}
f(3)
&=
...
\end{align*}
%%
This means that $f(3) = 3^2 = 9 = (-3)^2 = f(-3)$.
\end{solution}
}{}
...
\caption{%%
  Sketching a graph of a quadratic function.  First, locate the vertex
  $\tuple{\alpha}{\beta}$ of the function.  From there, spread outward
  with a few points.  Finally, you connect the dots.  Note that the
  graph is symmetric about the vertex.
}
\label{fig:sketch_parabola}
\end{figure}

The problem now is: How are you to calculate the vertex of any
quadratic function?  If you have a quadratic function of the form
$f(x) = ax^2 + bx + c$, the vertex~(i.e.~the highest or lowest point)
of the function is located at the $x$-coordinate
%%
\begin{equation}
...
Let $x$ be a real variable and let $\triple{a}{b}{c} \in \RR$ be
...
\end{align*}
%%
The latter equation can be simplified to $x^2 = a$ by using
...
\begin{example}
Use the quadratic formula to help you sketch a graph of the function
$f(x) = x^2 - x - 2$.
\end{example}
...
\caption{%%
  A graph of the function $f(x) = x^2 - x - 2$ through three points.
...
\caption{%%
  A graph of the function $f(x) = -2x^2 + x + 3$ through three
  points.  The three points are the vertex
  $\tuple{\frac{1}{4}}{\frac{25}{8}}$ and the roots $\tuple{-1}{0}$
  and $\tuple{\frac{3}{2}}{0}$ of $f(x)$.  In this case, the vertex of
  $f(x)$ is its highest point.
}
...
\caption{%%
  A graph of the function $f(x) = -x^2 + 1$ through three points.  The
...
strategy from \Section{sec:graphing} to sketch a graph of $f(x)$.  If
the discriminant of $f(x)$ is positive, then the strategy
from \Section{sec:quadratic_formula} can be used to draw a graph of
...
without actually calculating the roots of $f(x)$.  How many times does
the graph of $f(x)$ intersect the $x$-axis?
\end{exercise}
%%
\ifbool{showSolution}{
\begin{solution}
...
\item Another way to sketch a graph of a quadratic function
...
\caption{%%
  A graph of the function $f(x) = x^2 + x + 1$ through two points.
  The two points are the vertex and $y$-intercept of $f(x)$.
}
...
\caption{%%
  A graph of the function $f(w) = (4 - w)w$.  Here, $w$ denotes the
...
\item Consider the function
  ...
  x^2 - bx + 1
  \]
  where $b$ is a fixed number.  Determine all values of $b$ for which
  the graph of $f(x)$ does not intersect the $x$-axis.
\ifbool{showSolution}{
\begin{solution}
To determine whether the graph of a quadratic function intersects the
horizontal axis, you must calculate the discriminant of the function.
The discriminant of the given function is
\[
\Delta
=
(-b)^2 - 4(1)(1)
...
b^2 - 4.
\]
Note that if $\Delta < 0$, then the graph of $f(x)$ does not intersect
the horizontal axis.  In other words, you want to determine all values
of $b$ such that
%%
\begin{equation}
\label{eqn:negative_discriminant_b}
b^2 - 4
<
0.
\end{equation}
%%
This is equivalent to determining all values of $b$ such that
$b^2 < 4$.  You have three possibilities for $b$:
%%
\begin{packedenumeral}
\item\label{case:b_zero}
  The value of $b$ is zero.  If $b = 0$, then you have $0^2 < 4$,
  which is true.  So $b$ can be zero.

\item\label{case:b_positive}
  The value of $b$ is positive.  If $b > 0$, then you know that
  $2^2 = 4$, which is not what you want because you want $b^2$ to be
  less than $4$.  That is, the value of $b$ must be less than $2$ but
  greater than zero, or in symbols $0 < b < 2$.

\item\label{case:b_negative}
  The value of $b$ is negative.  If $b < 0$, then you know that
  $(-2)^2 = 4$.  So $b$ cannot be $-2$.  If $b < -2$, then the square
  $b^2$ would be a positive number that is greater than $4$.  So $b$
  can be neither $-2$ nor less than $-2$.  The only possibility for
  $b$ is to be greater than $-2$, but less than zero.  In symbols, you
  have $-2 < b < 0$.
\end{packedenumeral}
%%
According to the above three cases, in order for
\Expression{eqn:negative_discriminant_b} to be true, the possible
values of $b$ must be $-2 < b < 2$.  In terms of absolute values, the
latter expression can be written as $\absoluteValue{b} < 2$.
Therefore if the values of $b$ are given by $-2 < b < 2$, then the
graph of the function $f(x)$ does not intersect the horizontal axis.
\end{solution}
}{}
\end{problem}
