...
\node[left] (minusTwo) at (axis cs:-1,15) {$\tuple{\alpha-2}{h}$};
\node[] (B) at (-1,15) {};
%%
\node[left] (minusOne) at (axis cs:-1,8.25) {$\tuple{\alpha-1}{g}$};
\node[] (C) at (0.5,8.25) {};
\draw[->,thick] (minusOne) -- (C);
%%
\node[below] at (axis cs:2,6) {$\tuple{\alpha}{\beta}$};
%%
\node[right] (plusOne) at (axis cs:5,8.25) {$\tuple{\alpha+1}{d}$};
\node[] (E) at (3.5,8.25) {};
\draw[->,thick] (plusOne) -- (E);
%%
\node[right] (plusTwo) at (axis cs:5,15) {$\tuple{\alpha+2}{e}$};
\node[] at (5,15) {};
\end{axis}
\end{tikzpicture}
